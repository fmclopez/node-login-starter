...
if (errors.isEmpty()) {
  ...
  userModel.getOne({email:email} , (err, result) => {
    if (result) {
      ...
      const saltRounds = 10; //parameter needed by bcrypt

  // Hash password
  //bcrypt returns the hashed password thru the callback (2nd param)
  bcrypt.hash(password, saltRounds, (err, hashed) => {
    const newUser = {
      name, //omitted "name:" if same 
      email,
      password: hashed
    };
  ...
  //diff kind of object so still need to convert to array
  //express validator type of object
  //extracting mssg only
  req.flash('error_msg', messages.join(' ')); //setter
  res.redirect('/register');
}
};
      ...
      if (user) {
       ...
       bcrypt.compare(password, user.password, (err, result) => {
        ...
        if (result) { //boolean value 
            ...
            req.session.user = user._id; //updating session saved in database
            req.session.name = user.name;
            //if cookie, session id only
